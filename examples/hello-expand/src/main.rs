...
// meta "get_hello_world" ["METADATA:nidrs::datasets::RouterPath"]
// route_derive "get_hello_world"
// meta "create_user" ["METADATA:CreateUserDto::schema"]
...
mod modules {
    ...
    pub mod user {
        ...
        pub mod controller {
            ...
            use nidrs::{externs::axum::extract::Query, meta, post, valid::validator::Validator};
            use nidrs::{
            ...
            impl UserController {
                ...
                pub fn __meta_create_user(&self) -> nidrs::Meta {
                    let mut meta = nidrs::Meta::new();
                    meta.set_data(CreateUserDto::schema());
        ...
        impl nidrs::Module for UserModule {
            fn init(self, mut ctx: nidrs::ModuleCtx) -> nidrs::ModuleCtx {
                ...
                ctx.register_interceptor("UserModule", "UserInterceptor", Box::new(std::sync::Arc::new(crate::import::UserInterceptor::default())));
                ...
                if ctx.register_controller("UserModule", "UserController", Box::new(std::sync::Arc::new(controller::UserController::default()))) {
                    ...
                    let t_interceptor_1 = ctx.get_interceptor::<crate::import::UserInterceptor>("UserModule", "UserInterceptor");
                    let mut meta = nidrs::get_meta(t_controller.clone());
                ...
                let ctx = t.inject(ctx, &"UserModule");
                let t = ctx.get_interceptor::<UserInterceptor>("UserModule", "UserInterceptor");
                {
                    ::std::io::_print(format_args!("{0} ", nidrs_extern::colored::Colorize::green("[nidrs]"),));
                };
                {
